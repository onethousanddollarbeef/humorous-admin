...
async function createImage(formData: FormData) {
  ...
  try {
    ...
    const { error } = await adminSupabase.from("images").insert(payload);
    if (error) {
      redirect(imageRouteWithStatus({ error: `Image row insert failed. ${error.message}` }));
    }
...
async function updateImage(formData: FormData) {
  ...
  try {
    ...
    const { error } = await adminSupabase.from("images").update(payload).eq("id", id);
    if (error) {
      redirect(imageRouteWithStatus({ error: `Image update failed. ${error.message}` }));
    }
